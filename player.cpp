...
player::player() {
 ...
 SDL_Texture* player::load(SDL_Renderer* ren) {
	 SDL_Surface* pSur = IMG_Load("player");
	 if (!pSur) { std::cout << "failed to load image! img_error: " << SDL_GetError() << std::endl; }
	 SDL_Texture* pTex = SDL_CreateTextureFromSurface(ren, pSur);
	 if (!pTex) { std::cout << "failed to create texture! sdl_error: " << SDL_GetError() << std::endl; }
	 SDL_DestroySurface(pSur);
	 return pTex;
 }
